...
def build_clarification_suffix(session) -> str:
    ...
    if session._clarification_rounds == 0:
        return "\n\nИНСТРУКЦИЯ: Пользователь ответил на твои уточняющие вопросы. Если ответов достаточно для улучшения промпта — дай улучшенную версию и закончи [ОЖИДАЕТСЯ ВЫБОР]. Если критически не хватает — задай ещё вопросы и закончи [ОЖИДАЕТСЯ УТОЧНЕНИЕ]."
